        ...
        try:
            ...
            while self.is_running:
                ...
                if result[0] is not None:
                    frame, count, _, total_biomass = result
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.predict_button.setIcon(QIcon.fromTheme("view-refresh")) # Icon tạm
        ...
        self.recommendation_label = QLabel("AI Đề Xuất: Đang phân tích...")
    ...
    def update_frame(self):
        try:
            ...
            if not self.fish_counter.is_counting and not self.showing_final_result:
                return
            ...
            # Update Activity Bar
            self.activity_bar.setValue(int(fai))
            ...
            if has_dead_fish:
                ...
                status = "LỜ ĐỜ (Low)"
                color = "#888888" # Grey
                rec_text = "⚠️ Cảnh báo: Kiểm tra Oxy hoặc Sức khỏe cá"
                rec_color = "#aaaaaa"
            elif fai < 60:
                status = "BÌNH THƯỜNG (Normal)"
                color = "#00ff00" # Green
                rec_text = "✅ Trạng thái ổn định"
                rec_color = "#00ff00"
            elif fai < 80:
                status = "SUNG SỨC (High)"
                color = "#ffaa00" # Orange
                rec_text = "🍽️ Cá đang đói: Thời điểm tốt để cho ăn"
                rec_color = "#ffaa00"
            else:
                status = "NÁO LOẠN (Frenzy)"
                color = "#ff0000" # Red
                rec_text = "🛑 Cảnh báo: Dừng cho ăn / Giảm lượng thức ăn"
                rec_color = "#ff5555"
            ...
            self.recommendation_label.setText(f"AI Đề Xuất: {rec_text}")
    ...
    def plot_history(self):
        try:
            ...
            for result in self.count_history:
                try:
                    timestamps.append(datetime.strptime(result['timestamp'], "%Y-%m-%d %H:%M:%S"))
                    counts.append(int(result['count']))
                except: continue

            if timestamps:
                self.ax.plot(timestamps, counts, 'o-', color='#4db8ff', linewidth=2)
    ...
    def show_detail_view(self):
        try:
            ...
            # --- CỘT 2: SINH KHỐI ---
            bio_group = QGroupBox("THỐNG KÊ SINH KHỐI")
            bio_layout = QFormLayout(bio_group)
            
            valid_bio = [b for b in all_biomass if b > 0]
            if valid_bio:
                b_min = min(valid_bio)
                b_max = max(valid_bio)
