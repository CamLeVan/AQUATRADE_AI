...
from . import metrics
from .deps import rate_limit_jobs, require_internal_secret
...
from .store import IdempotencyConflictError, get_store
...
# Sprint 7-LITE: thêm `sha256` (hash file weights) cho audit/dispute.
_MODEL_STATE: dict[str, object] = {"loaded": False, "sha256": None}
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        ...
        # Sprint 7-LITE: tính SHA-256 file weights để audit dispute.
        _MODEL_STATE["sha256"] = await _compute_model_sha256(settings.model_path)
        logger.info(
            "model_loaded",
            path=settings.model_path,
            version=settings.model_version,
            sha256=_MODEL_STATE["sha256"],
        )
...
async def _compute_model_sha256(path: str) -> str | None:
    """Sprint 7-LITE: SHA-256 hex của file model weights cho audit dispute.

    Run trong thread vì có thể đọc file vài MB-vài chục MB.
    """
    import asyncio
    import hashlib
    import os
...
def create_app() -> FastAPI:
    ...
    async def list_models(
        ...
        info = ModelInfo(
            ...
            active=True,
            loaded_at=loaded_at,
            sha256=_MODEL_STATE.get("sha256"),
        )
    ...
    @app.post(
        ...
        tags=["jobs"],
        dependencies=[Depends(require_internal_secret), Depends(rate_limit_jobs)],
    )
    async def submit_job(
        ...
        # BE develop check `response.containsKey("ticketId")` ở ROOT level → expose
        # `ticketId` ở root đồng thời giữ wrapper {status, message, data} cho consistency.
        ...
        body = ApiResponse[JobAcceptedResponse](
            message="Job accepted" if is_new else "Job already in progress",
            data=data,
        ).model_dump(by_alias=True, mode="json", exclude_none=True)
        body["ticketId"] = record.ticket_id
        return JSONResponse(
            status_code=status.HTTP_202_ACCEPTED, content=body,
        )
    ...
    # ---- Dead-letter list (Sprint 5, internal) ----
    @app.get(
        "/ai/v1/dead-letters",
        tags=["jobs"],
        dependencies=[Depends(require_internal_secret)],
    )
    async def list_dead_letters(limit: int = 50) -> ApiResponse[dict]:
        store = get_store()
        items = await store.list_dead_letters(limit=max(1, min(500, int(limit))))
